...
# Start initial cron (disabled on GKE: one CronJob, not N keep-alive processes).
if is_boolean_yes "${MOODLE_CRON_ENABLED:-yes}"; then
    start_cron
    monitor_cron &
else
    info "In-container Moodle cron disabled (MOODLE_CRON_ENABLED=no)"
fi
